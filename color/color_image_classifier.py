...
class ImageClassifier:
    ...
    def get_database_histogram(self, database_path):
        ...
        color_pixles = np.array(Parallel(n_jobs=num_cores, prefer='processes')(delayed(self.img_color_pixels)(imgs_database[ii], self.database_path) for ii in range(len(imgs_database))))
        self.database_hist  = np.array(Parallel(n_jobs=num_cores, prefer='processes')(delayed(self.histogram_3d)(color_pixles[ii, :, :]) for ii in range(len(imgs_database))))
    ...
    def compare_image(self, query_image='ironman', metric='emd'):
        ...
        if method <= 5:
            ...
            self.distances = np.array(Parallel(n_jobs=num_cores, prefer='processes')(delayed(bin2bin_hist_comp)(hist1, self.database_hist[ii, :, :, :], method) for ii in range(len(self.classes))))

        if method == 6:
            hist1 = query_hist
            self.distances = np.array(Parallel(n_jobs=num_cores, prefer='processes')(delayed(ot_hist_comp)(hist1, self.database_hist[ii, :, :, :]) for ii in range(len(self.classes))))
